...
gestion_lev::gestion_lev(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::gestion_lev)
{
ui->setupUi(this);
...
gestion_lev::~gestion_lev()
{
    delete ui;
}

void gestion_lev::on_pb_ajouter_3_clicked()
{
...
void gestion_lev::on_pb_modifier_3_clicked()
{
...
void gestion_lev::on_pb_supprimer_3_clicked()
{
...
void gestion_lev::on_pb_ajouter_4_clicked()
{
...
void gestion_lev::on_pb_modifier_4_clicked()
{
...
void gestion_lev::on_pb_supprimer_4_clicked()
{
...
void gestion_lev::on_pushButton_clicked()
{
    ...
    QString strStream;
        ...
        const int rowCount = ui->tabLocalisation_4->model()->rowCount();
        ...
        for (int column = 0; column < columnCount; column++)
            if (!ui->tabLocalisation_4->isColumnHidden(column))
                out << QString("<th>%1</th>").arg(ui->tabLocalisation_4->model()->headerData(column, Qt::Horizontal).toString());
        out << "</tr></thead>\n";
        ...
        for (int row = 0; row < rowCount; row++) {
            ...
            for (int column = 0; column < columnCount; column++) {
                if (!ui->tabLocalisation_4->isColumnHidden(column)) {
                    QString data = ui->tabLocalisation_4->model()->data(ui->tabLocalisation_4->model()->index(row, column)).toString().simplified();
...
void gestion_lev::on_pushButton_4_clicked()
{
    statt statt;
      statt.setModal(true);
      statt.exec();
}
void gestion_lev::on_pushButton_3_clicked()
{
    statt statt;
      statt.setModal(true);
      statt.exec();
}
void gestion_lev::on_tab_5_destroyed()
{
